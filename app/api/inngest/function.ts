...
export const handleJobExpiration = inngest.createFunction (
    ...
    async ({ event, step }) => {
        ...
        await step.sleep('wait-for-expiration', `${1}d`);
...
export const sendPeriodJobListings = inngest.createFunction(
       ...
       async ({ event, step }) => {
        ...
        while(currentDay < totalDays) {
            ...
            const jobListingsHtml = recentJobs
                ...
                (job) => `
                    ...
                    <p style="margin: 5px 0;">$${job.salaryForm.toLocaleString()} - $${job.salaryTo.toLocaleString()}</p>
                    </div>
                `
                        )
                        .join("");
        ...
        return {userId, message: "Completed 30 days job listings notification"}
